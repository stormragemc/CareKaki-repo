...
The repo is now **deploy-ready**: the frontend reads its backend URL from
...
## What works on the public link vs. not

| Feature | Public deploy |
|---|---|
| AiMao home, chat, care plan, care brief, expert view | ✅ |
| LLM chat + profile + plan generation | ✅ (uses your OpenAI key) |
| Real SG data adapters (AIC / nursing / medication maps) | ✅ |
| ElevenLabs voice | ✅ until free characters run out |
| Telegram bot alerts to a real phone | ⚠️ only if you add the Telegram env vars + a public webhook URL |
