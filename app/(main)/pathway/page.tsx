...
import CarePlanDocument from "@/components/careplan/CarePlanDocument";
...
export default function PathwayPage() {
  ...
  if (userRaw) {
    try {
      const demoUser = JSON.parse(userRaw) as DemoUser;
      personaId = demoUser.id;
  ...
  const suggestions = tl("plan.suggestions");
  ...
  const title = patientName ? t("plan.titleOf", { name: patientName }) : t("plan.titleOwn");
  ...
  const sendEdit = async (raw: string) => {
    ...
    try {
      const res = await fetch(apiUrl("/pathway/edit-items"), {
        ...
        body: JSON.stringify({ items, mode, feedback, language: lang }),
      });
      ...
      setMessages((prev) => [
        ...
        {
          role: "system",
          text: data.edited === false ? t("plan.editOffline") : t("plan.editDone"),
        },
      ]);
    } catch {
      setMessages((prev) => [
        ...prev,
        { role: "system", text: t("plan.editUnreachable") },
      ]);
    } finally {
      setEditing(false);
    }
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="mx-auto flex w-full max-w-7xl gap-6 px-6 py-10">
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-8">
          <div className="print-hidden flex items-start justify-between gap-4">
            <div className="flex flex-col gap-1">
              ...
              <p className="mt-1 text-ink-soft">{t("plan.desc")}</p>
            </div>
            <div className="flex shrink-0 items-center gap-3">
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-full border border-hairline px-4 py-2 text-sm font-medium text-ink-body transition-colors hover:border-aimao-teal hover:text-aimao-teal"
              >
              ...
              >
                {chatOpen ? t("plan.closeEditor") : t("plan.editPlan")}
              </button>
            </div>
          </div>
            ...
            <div className="flex items-start gap-3 rounded-2xl border border-caregiver-border bg-caregiver-soft px-5 py-4">
              ...
              <div className="flex flex-col gap-1">
                <p className="text-sm font-semibold text-caregiver-ink">
                  {t("plan.carriedTitle", { phase: lastPhase.phase })}
                </p>
                <p className="text-sm leading-relaxed text-ink-body">{lastPhase.summary}</p>
                <p className="text-xs text-ink-muted">{t("plan.carriedNote")}</p>
              </div>
            </div>
          )}
          ...
          <CarePlanDocument
            ...
            preparedWith={
              userName
                ? `${userName.split(" ")[0]} ${t("doc.andAiMao")}`
                : t("doc.preparedWithFamily")
            }
          />
        </div>
        ...
        {chatOpen && (
          <aside className="print-hidden flex w-80 shrink-0 flex-col self-start rounded-2xl border border-hairline bg-surface">
            <div className="flex items-center justify-between border-b border-hairline px-4 py-3">
              <div>
                <p className="text-sm font-semibold text-ink">{t("plan.editorTitle")}</p>
                <p className="text-[11px] text-ink-muted">{t("plan.editorSub")}</p>
              </div>
              <button
            ...
            <div className="flex max-h-[60vh] flex-1 flex-col gap-3 overflow-y-auto px-4 py-3">
              {messages.length === 0 && (
                <div className="flex flex-col gap-2">
                  <p className="text-[11px] text-ink-muted">{t("plan.tryThese")}</p>
                  {suggestions.map((s) => (
                    <button
                      key={s}
                      onClick={() => sendEdit(s)}
              ...
              {editing && (
                <div className="flex items-center gap-2 px-1">
                  ...
                  <span className="text-[11px] text-ink-muted">{t("plan.updating")}</span>
                </div>
              )}
            ...
            <div className="border-t border-hairline px-3 py-3">
              <div className="flex gap-2">
                <input
                  ...
                  placeholder={t("plan.editorPlaceholder")}
                  disabled={editing}
      ...
      {/* Footer bar — AiMao arranges everything behind the scenes */}
      <div className="print-hidden border-t border-hairline bg-surface">
        <div className="mx-auto flex w-full max-w-7xl flex-col items-center justify-between gap-4 px-6 py-5 sm:flex-row">
          <p className="text-base text-ink-body">{t("plan.footerText")}</p>
          <Link
            href="/autopilot"
            className="inline-flex min-h-[54px] shrink-0 items-center gap-2 rounded-full bg-aimao-teal px-7 text-base font-semibold text-white transition-colors hover:bg-aimao-teal-ink"
          >
            {t("plan.footerCta")}
          </Link>
        </div>
