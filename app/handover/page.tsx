...
import { Check, ShieldCheck, Star, ArrowRight, AlertTriangle, Info } from "lucide-react";
              ...
              <section className="flex flex-col gap-2.5">
                ...
                <ul className="flex flex-col gap-2">
                  {brief.next_steps.map((step, i) => (
                    <li key={i} className="flex items-start gap-2.5 text-ink-body text-sm">
                      <ArrowRight size={16} className="mt-0.5 shrink-0 text-brand-teal" aria-hidden="true" />
                      <span>{step}</span>
                    </li>
                  ))}
                </ul>
