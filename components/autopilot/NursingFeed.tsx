...
export default function NursingFeed({ enabled = true }: { enabled?: boolean }) {
  ...
  if (!enabled) return <DraftNotice label="Drafted — provider booking awaiting approval" />;
...
function ProviderCard({ rank, provider }: { rank: number; provider: Provider }) {
  return (
    <div className="rounded-lg border border-autopilot-hairline bg-autopilot-band px-3 py-2.5 flex flex-col gap-2">
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          <span className="shrink-0 w-5 h-5 rounded-full bg-weeks/30 text-weeks-soft text-[10px] font-bold flex items-center justify-center">
            {rank}
          </span>
          <span className="text-xs font-semibold text-autopilot-text truncate">{provider.name}</span>
        </div>
        {provider.distance_km != null && (
          <span className="shrink-0 text-[10px] text-autopilot-muted">{provider.distance_km} km</span>
        )}
      </div>

      <div className="pl-7 flex flex-col gap-0.5">
        <span className="text-[11px] text-autopilot-muted leading-snug">{provider.address}</span>
        {provider.phone && (
          <span className="text-[10px] text-autopilot-muted">Tel: {provider.phone}</span>
        )}
      </div>

      <div className="pl-7 flex flex-wrap gap-1">
        {provider.programmes.map((p) => (
          <span key={p} className="text-[9px] px-1.5 py-0.5 rounded bg-white/[0.06] text-autopilot-muted">
            {p}
          </span>
        ))}
      </div>
      ...
      <div className="pl-7 flex flex-col gap-1">
        <span className="text-[10px] text-autopilot-muted uppercase tracking-wider">Available slots</span>
        {provider.available_slots.map((slot, i) => (
          <div key={i} className="flex items-center gap-2">
            <span className="w-1 h-1 rounded-full bg-status-done-dark shrink-0" />
            <span className="text-[11px] text-autopilot-text">{slot.date} · {slot.time}</span>
            <span className="text-[10px] px-1.5 py-0.5 rounded bg-weeks/20 text-weeks-soft">{slot.type}</span>
          </div>
        ))}
      </div>
      ...
      <div className="pl-7 flex items-center gap-1.5">
        ...
        <span className="text-[10px] text-status-running-dark">Tentative booking</span>
      </div>
    </div>
  );
}
