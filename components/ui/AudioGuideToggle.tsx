...
export default function AudioGuideToggle({
  ...
  const accent = dark ? "text-brand-teal" : "text-brand-teal";

  if (!enabled) {
    return (
      <button
        onClick={onEnable}
        className={`flex items-center gap-2 px-3 py-1.5 rounded-full border text-xs font-medium transition-colors hover:border-brand-teal/50 ${base}`}
      >
        ...
        <div className="flex items-center gap-0.5 ml-1">
          ...
          <button
            ...
            className={`p-1 rounded-md text-sm transition-colors ${
              micOn ? "bg-brand-orange/20 text-brand-orange" : `hover:bg-white/10 ${muted}`
            } disabled:opacity-30`}
